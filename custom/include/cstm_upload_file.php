...
class CstmUploadFile
{
	...
	function final_move($bean_id)
	{
        ...
        if($this->use_soap) {
			...
			if(!CstmUploadStream::move_uploaded_file($_FILES[$this->field_name]['tmp_name'], $destination)) {
    ...
    public static function realpath($path)
    {
       if(substr($path, 0, 9) == "upload://") {
           $path = CstmUploadStream::path($path);
       }
       $ret = realpath($path);
       return $ret?$ret:$path;
    }
...
class CstmUploadStream
{
    ...
    public static function getSuhosinStatus()
    {
        ...
        // checking that CstmUploadStream::STREAM_NAME is allowed by white list
        ...
        if ($streams != '')
        {
            ...
            foreach($streams as $stream)
            {
                ...
                if (count($stream) == 1)
                {
                    if ($stream[0] == CstmUploadStream::STREAM_NAME)
                    {
                        return true;
                    }
                }
                elseif ($stream[1] == '' && $stream[0] == CstmUploadStream::STREAM_NAME)
                {
                    return true;
                }
            }

            $GLOBALS['log']->fatal('Stream ' . CstmUploadStream::STREAM_NAME . ' is not listed in suhosin.executor.include.whitelist and blocked because of it');
            return false;
        }

        // checking that CstmUploadStream::STREAM_NAME is not blocked by black list
        ...
        if ($streams != '')
        {
            ...
            foreach($streams as $stream)
            {
                ...
                if ($stream[0] == CstmUploadStream::STREAM_NAME)
                {
                    $GLOBALS['log']->fatal('Stream ' . CstmUploadStream::STREAM_NAME . 'is listed in suhosin.executor.include.blacklist and blocked because of it');
                    return false;
                }
            }
            return true;
        ...
        $GLOBALS['log']->fatal('Suhosin blocks all streams, please define ' . CstmUploadStream::STREAM_NAME . ' stream in suhosin.executor.include.whitelist');
        return false;
    }
